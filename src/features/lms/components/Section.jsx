...
export default function SubDepartment() {
  ...
  if (!department) {
    return (
      <div className="flex min-h-[calc(100vh-70px)] items-center justify-center bg-[#F5F7FB] text-[#26364d]">
        <div className="text-center">
          <div className="mx-auto mb-4 flex h-[45px] w-[45px] items-center justify-center rounded-xl bg-[#f0ecff]">
            <Layers size={21} className="text-[#6F4AE7]" />
          </div>
          <h2 className="text-[15px] font-bold">Department not found</h2>
          <p className="mt-1 text-xs text-[#718096]">
            The selected department could not be found.
          </p>
          <button
  ...
  const orderedSubDepartments = subDepartments
    .map((subDept, index) => ({ ...subDept, displayId: index + 1 }))
    .slice();

  const filteredSubDepartments = orderedSubDepartments.filter((subDept) => {
    const query = search.trim().toLowerCase();
    if (!query) return true;
    return (
      String(subDept.displayId).includes(query) ||
      subDept.name.toLowerCase().includes(query)
    );
  });

  const updateDepartment = (updater) => {
    const next = departments.map((dept) =>
      String(dept.id) === String(deptId) ? updater(dept) : dept,
    );
    setDepartments(next);
  ...
  const saveSubDepartment = () => {
    ...
    if (!name) {
      showToast("Please enter sub-department name.");
      return;
    }

    const isDuplicate = subDepartments.some(
      (subDept) => subDept.name.toLowerCase() === name.toLowerCase(),
    );

    if (isDuplicate) {
      showToast("Sub-department already exists.");
      return;
    }
    ...
    updateDepartment((dept) => ({
      ...
      subDepartments: [
        ...(dept.subDepartments || []),
        { id: createId("subdept"), name, lines: [] },
      ],
    }));

    showToast("Sub-department added successfully.");
    setAddModal(false);
  };
  ...
  const openEditModal = (subDept) => {
    setEditTarget(subDept);
    setEditName(subDept.name);
    setEditModal(true);
  };
  ...
  const saveEdit = () => {
    ...
    updateDepartment((dept) => ({
      ...
      subDepartments: (dept.subDepartments || []).map((subDept) =>
        subDept.id === editTarget.id ? { ...subDept, name } : subDept,
      ),
    }));

    showToast("Sub-department updated successfully.");
    setEditModal(false);
  };

  /* ===================== DELETE SUB-DEPARTMENT ===================== */

  const askDelete = (subDept) => {
    setDeleteTarget(subDept);
    setDeleteModal(true);
  };
  ...
  const confirmDelete = () => {
    ...
    updateDepartment((dept) => ({
      ...
      subDepartments: (dept.subDepartments || []).filter(
        (subDept) => subDept.id !== deleteTarget.id,
      ),
    }));
    ...
    <div className="text-[#26364d]">
      <section className="p-4 sm:p-[30px_25px]">
        ...
        <div className="mb-3 flex items-center gap-1.5 text-xs text-[#9aa3af]">
          ...
          <span>/</span>
          <span className="text-[#718096]">{department.name}</span>
          <span>/</span>
          <span className="font-semibold text-[#26364d]">Sub-Departments</span>
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <h1 className="text-[15px] font-bold">
                  Sub-Departments: {department.name}
                </h1>
                <p className="mt-0.5 text-xs text-[#718096]">
                  Manage sub-departments within this department
                </p>
              </div>
            </div>
                        ...
                        <td className="px-3 py-3 sm:px-4">
                          <div className="flex items-center justify-end gap-2">
                            <button
                              onClick={() =>
                                navigate(
                                  `/lms/line/${department.id}/${subDept.id}`,
                                )
                              }
      ...
      {addModal && (
        ...
        >
          <div className="max-h-full w-full max-w-[480px] overflow-hidden rounded-[17px] bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#e3e6eb] px-5 py-[18px]">
              <h2 className="text-base font-bold">Add Sub-Department</h2>
              <button
            ...
            <div className="p-5">
              ...
              <input
                ...
                onKeyDown={(event) =>
                  event.key === "Enter" && saveSubDepartment()
                }
      ...
      {editModal && (
        ...
        >
          <div className="max-h-full w-full max-w-[480px] overflow-hidden rounded-[17px] bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#e3e6eb] px-5 py-[18px]">
              <h2 className="text-base font-bold">Edit Sub-Department</h2>
              <button
          ...
          <div className="max-h-full w-full max-w-[480px] overflow-hidden rounded-[17px] bg-white shadow-2xl">
            ...
            <div className="p-5">
              <p className="text-sm leading-6 text-gray-600">
                ...
                ? This will also remove all of its sections and lines.
              </p>
