...
const Machine = () => {
  const navigate = useNavigate();
  const { deptId, subDeptId, lineId } = useParams();
  ...
  // The line opened by the URL (/lms/machine/:deptId/:subDeptId/:lineId)
  const department = departments.find(
    (dept) => String(dept.id) === String(deptId),
  );
  ...
  const filteredMachines = machines.filter((machine) => {
    ...
    const matchesSearch =
      !value ||
      [machine.name, machine.code, machine.type, machine.status].some((field) =>
        String(field || "")
          .toLowerCase()
          .includes(value),
      );
  ...
  const saveMachine = () => {
    ...
    const duplicate = departments.some((dept) =>
      (dept.subDepartments || []).some((subDept) =>
        (subDept.lines || []).some((currentLine) =>
          (currentLine.machines || []).some(
            (machine) =>
              String(machine.code || "").toLowerCase() === code.toLowerCase(),
          ),
    ...
    if (duplicate && !editingMachine) {
      showToast("Machine code already exists.");
      return;
    }

    const machine = {
      id: editingMachine?.id || createId("machine"),
      name,
      code,
      type: form.type,
      operators: Number(form.operators) || 0,
        ...
        : {
            ...
            subDepartments: (dept.subDepartments || []).map((subDept) =>
              String(subDept.id) !== String(subDeptId)
                ...
                : {
                    ...subDept,
                    lines: (subDept.lines || []).map((currentLine) => {
                      const isTarget =
                        String(currentLine.id) === String(lineId) ||
                        String(currentLine.code) === String(lineId);
  ...
  if (!department || !subDepartment || !line) {
    return (
      <div className="flex min-h-[calc(100vh-70px)] items-center justify-center bg-[#F5F7FB] text-[#26364d]">
        <div className="text-center">
          <h2 className="text-[15px] font-bold text-[#26364d]">
  ...
  return (
    <div className="text-[#26364d]">
      <section className="p-4 sm:p-[30px_25px]">
        {/* BREADCRUMB */}
        <div className="mb-3 flex flex-wrap items-center gap-1.5 text-xs text-[#9aa3af]">
          <Link
            to="/lms/department"
            className="hover:text-[#6c4ce8] hover:underline"
          >
            Departments
          </Link>
          <span>/</span>
          ...
          <span>/</span>
          <Link
            to={`/lms/line/${deptId}/${subDeptId}`}
            className="hover:text-[#6c4ce8] hover:underline"
          >
            {subDepartment.name}
          </Link>
          <span>/</span>
          <span className="font-semibold text-[#26364d]">{line.name}</span>
        </div>

        <div className="overflow-hidden rounded-[17px] border border-[#e3e6eb] bg-white shadow-sm">
          {/* HEADER */}

          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#edf0f3] px-4 py-4 sm:px-5 sm:py-[18px]">
            <div className="flex items-center gap-3">
              <div className="flex h-[45px] w-[45px] items-center justify-center rounded-xl bg-gradient-to-br from-[#6c4ce8] to-[#8b6ffe] text-white shadow-md shadow-[#6c4ce8]/30">
                <Cpu size={21} />
              </div>
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => navigate(`/lms/line/${deptId}/${subDeptId}`)}
                className="flex h-[38px] items-center gap-1.5 rounded-lg border border-[#e3e6eb] bg-white px-4 text-xs font-semibold text-[#718096] transition hover:bg-[#f7f8fa]"
              >
                <ArrowLeft size={15} />
                Back to Lines
          ...
          <div className="grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 sm:p-5">
            <div className="rounded-[14px] border border-[#e3e6eb] bg-[#f7f8fa] px-5 py-4">
              ...
              <p className="mt-1 text-lg font-bold text-[#6c4ce8]">
                {line.name}
              </p>
            </div>
                  ...
                  {filteredMachines.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="p-0">
                        <div className="flex flex-col items-center justify-center py-16 text-[#9aa3af]">
                          <Cpu size={42} className="mb-3 opacity-40" />
                          <p className="mb-4 text-sm">No machines found.</p>
                          <button
                            type="button"
                            onClick={openAddModal}
                            className="flex items-center gap-2 rounded-lg bg-[#6c4ce8] px-4 py-2 text-xs font-semibold text-white"
                          >
                            <Plus size={15} />
                            Add Machine
                          </button>
                      ...
                      return (
                        ...
                        >
                          <td className="border-r border-[#edf0f2] px-3 py-3 last:border-r-0 sm:px-4">
                            <span className="inline-flex rounded-full bg-[#f0ecff] px-2.5 py-1 text-[12px] font-semibold text-[#6c4ce8]">
                              #{index + 1}
                            </span>
                          </td>
